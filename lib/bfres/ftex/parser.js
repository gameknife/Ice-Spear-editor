...
module.exports = class FTEX_Parser
{
    ...
    async parse()
    {
        ...
        try{
            ...
            this.header = await FTEX_Cache.getCached(this.entry.fileName, () => this._loadTextureData());
    ...
    async _loadTextureData()
    {
        ...
        if(this.mipmapStartLevel > 0)
            this._applyMipmapRules(img);

        const rawBuffer = this.parser.file.buffer.slice(this.header.dataOffset, this.header.dataOffset + this.header.surface.dataSize);

        const imageBuffers = this._generateColorBuffer(img, texTypeInfo, rawBuffer);
        ...
        // gameknife
        //console.info(`FTEX::parse: ${this.entry.fileName} - ${img.sizeX}x${img.sizeY}x${img.sizeZ} - ${img.textureFormat} - ${img.mipmapCount} mipmaps`);
        // imagebuffers contain mipmap, clip to first mipmap
        const mip0Buffer = imageBuffers[0].slice(0, img.sizeX * img.sizeY * img.colorChannels)
        //let pngBuffer = PNG.encode(mip0Buffer, img.sizeX, img.sizeY);
        ...
        //if( this.mipmapStartLevel == 0 )
        {
            var png = new PNG({
                ...
                height: img.sizeY,
                filterType: -1,
                inputColorType: img.colorChannels == 4 ? 6 : 2,
            });
            png.data = mip0Buffer;
            png.pack().pipe(fs.createWriteStream(`./exported/${document.GK_MAP_NAME}/${this.entry.fileName}.png`));
        }


       // var options = { colorType: 6 };
        //var buffer = PNG.sync.write(mip0Buffer, options);
        // fs.writeFile( `./exported/${this.entry.fileName}.png`, mip0Buffer, (err) =>
        // {
    ...
    _generateColorBuffer(img, texTypeInfo, rawBuffer)
    {
        ...
        for(let index=0; index<img.sizeZ; ++index)
        {
            ...
            if(img.sizeZ > 1 && texData.colorChannels != orgChannels && !alphaRemoved)
            {
                alphaRemoved = true;
                console.warn("FTEX_Parse: array-texture removed alpha-channel!");
            }
