...
module.exports = class BFRES_Parser
{
    ...
    getModels()
    {
        ...
        if(fmdlFiles)
        {
            for(let entry of fmdlFiles.entries)
            {
                ...
                {     
                    models[entry.parser.header.fileName] = entry.parser.models;
                    let converter = new Converter_OBJ();
                    let filename = entry.parser.header.fileName;
                    if(entry.parser.header.fileName == "Base")
                    {
                        filename = `Base_${document.GK_MAP_NAME}`;
                    }
                    const objFileName = `./exported/global/${filename}.obj`;
                    if(!fs.existsSync(objFileName))
                    {
                        converter.convert(entry.parser.models, `./exported/global/`, filename);   
                    }
                }
            }
        }
    ...
    getTextureByName(name)
    {
        ...
        if(ftexFiles)
        {
            ...
            {
                if(entry.parser && entry.parser.header)
                {
