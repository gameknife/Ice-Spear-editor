        ...
        if(fmdlFiles)
        {
            for(let entry of fmdlFiles.entries)
            {
                ...
                {     
                    ...
                    console.info(`[gk] export model: ${entry.parser.header.fileName}`);  
                    let converter = new Converter_OBJ();
                    converter.convert(entry.parser.models, `./exported/`, entry.parser.header.fileName);     
                }
            }
        }
        ...
        if(ftexFiles)
        {
            for(let entry of ftexFiles.entries)
            {
                if(entry.parser && entry.parser.header)
                {
                    ...
                    {
                        console.info(`[gk]tex: ${entry.parser.header.fileName}`);
