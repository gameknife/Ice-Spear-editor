...
// as lod5, 256x256 section will scale to 500m x 500m, so the 3x256 tile will be 1500m x 1500m, not 768m x 768m, scale number will be 1.953125
// the height scale is 0.0122075, 16bit max value is 65535, so the peak is 65535 * 0.0122075 = 800m, from ground to heightest peak is 800m, the unreal z scale will be 800 / 512 = 1.5625
...
module.exports = class Terrain
{
    ...
    async loadSectionMesh(sectionName, lodLevel = LOD_LEVEL_MAX)
    {
        ...
        for(const tile of sectionTiles)
        {
            ...
            console.info( `create Tile: name: ${tile.name} center: ${tile.center} address: [${tx}, ${ty}] lodscale: ${tile.lodScale} hmax: ${tile.groundHeightMax} hmin: ${tile.groundHeightMin}` );
            ...
            // right the mate map   one pixel with 4 byte, blend 0, blend 1, blend weight, unknown
            bufferIndex = 0;
            for(let y=0; y<MAP_TILE_LENGTH; ++y)
            {
                for(let x=0; x<MAP_TILE_LENGTH; ++x)
                {
                    document.GlobalWeightMap[(ty * 256 + y) * (256 * 20 * 4) + (tx * 256 + x) * 4 + 0] = materialBuffer[bufferIndex+0];
                    document.GlobalWeightMap[(ty * 256 + y) * (256 * 20 * 4) + (tx * 256 + x) * 4 + 1] = materialBuffer[bufferIndex+1];
                    document.GlobalWeightMap[(ty * 256 + y) * (256 * 20 * 4) + (tx * 256 + x) * 4 + 2] = materialBuffer[bufferIndex+2];
                    document.GlobalWeightMap[(ty * 256 + y) * (256 * 20 * 4) + (tx * 256 + x) * 4 + 3] = materialBuffer[bufferIndex+3];
                    bufferIndex += 4;
                }
            }
        ...
        console.info( `loaded section: ${sectionName}  lod: ${lodLevel} renderScale: ${tileSectionScale}` );
