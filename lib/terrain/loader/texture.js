...
async function loadTextureFromBfres(terrainTexPath)
{
    ...
    // gameknife, export terrain textures
    // gameknife
    let index = 0;
    for( const img of ftexSurface.imageBuffers )
    {
        const pngFileName = `./exported/TerrainAlb_${index}.png`;
        if(!fs.existsSync(pngFileName))
        {
            const mip0Buffer = img.slice(0, ftexSurface.sizeX * ftexSurface.sizeY * ftexSurface.colorChannels)
            var png = new PNG({
                width: ftexSurface.sizeX,
                height: ftexSurface.sizeY,
                filterType: -1,
                inputColorType: ftexSurface.colorChannels == 4 ? 6 : 2,
            });
            png.data = mip0Buffer;
...
/**
 * packs all image buffers into one and removes the alpha channel to save space
