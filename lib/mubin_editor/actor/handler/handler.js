    ...
    async addActor(name, params, type, alreadyIncluded = true)
    {
        ...
        if(type == "Static" || type == "Dynamic")
        {
            if('__DEFAULT_BOX' != modelName && mainModelName != "None")
            {
                ...
                const actorInfoRecord = document.actorInfo[name];

                document.export_ulevel[`${name}_${actor.id}`] = { actor: name, 
                    folder: actorInfoRecord ? actorInfoRecord.model.folder : "None",
                    mesh: actorInfoRecord ? actorInfoRecord.model.mesh : "None",
