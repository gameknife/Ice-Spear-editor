...
module.exports = class Actor_Object_Loader
{
    ...
    async init()
    {
        ...
        // gameknife, output all actor modellist
        for(let actorName in this.actorInfo)
        {
            let actorInfo = await this._getActorInfo(actorName);
            if(actorInfo.bfresContainer != undefined && actorInfo.mainModel != undefined)
            {
                let allmdl = actorInfo.bfresContainer.getModels();
                let modellist = '';
                for( let mdlkey in allmdl )
                {
                    modellist += mdlkey + ' ';
                }

                let mainModelName = actorInfo.mainModel.value;
                console.log(`actor: ${actorName} - models: ${modellist} - main: ${mainModelName}`);
            }
        }
    ...
    async _getActorObject(name, aliasName = undefined)
    {
        if(!this.objects[name])
        {
            ...
            // gameknife, add a fast file access
            ...
            objGroup.userData = {type: "actor", visible: true};
            ...
            // gameknife, add a fast file access
            this.objects[name].mainModelName = mainModelName;
        }
        ...
        {            
            if(info.bfres != null)
            {
                try{
                    ...
                    console.log("Invalid Actor-Model: " + name);
                    return undefined;
                }
            }else{
